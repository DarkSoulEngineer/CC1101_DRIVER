...
TX file (rfuzz_2fsk_gap.c8, generated by the `gen` subcommand).

Modulation constants (shared by the `gen` subcommand):
...
Usage:
    python rfuzz_tools.py gen    out.c8 --repeat 60 --gap-ms 50
...
# The TX signal generator (`rfuzz_tools gen`) sends sync 0xBEEF; SYNC_BITS
# above is
...
def packet_bits(preamble_bytes=DEFAULT_PREAMBLE_BYTES, payload=PAYLOAD_EXPECT):
    """Bit sequence for one TX packet: 0xAA preamble + 0xBEEF sync + payload,
    MSB-first (numpy array).  Absorbed from the former gen_2fsk.py."""
    return bits_msb(bytes([0xAA]) * preamble_bytes + MP_SYNC_BYTES + payload)


def _random_payload(length, seed):
    """Deterministic pseudo-random payload bytes for the gen CLI."""
    rng = np.random.default_rng(seed)
    return bytes(int(x) for x in rng.integers(0, 256, size=length, dtype=np.uint8))


def gen_2fsk_signal(out_path, repeat=100, gap_ms=0.0, fs=FS, dev=DEFAULT_DEV,
                    amp=DEFAULT_AMP, preamble_bytes=DEFAULT_PREAMBLE_BYTES,
                    payload=PAYLOAD_EXPECT, baud=BPS):
    """Generate the 2FSK test signal (int8 interleaved I/Q .c8) used by the
    RFuzz end-to-end testcase.  Each packet = 0xAA preamble + 0xBEEF sync +
    payload, bits MSB-first, +dev/-dev phase-continuous FSK; `gap_ms` inserts
    silent (zero-amplitude) idle between packets.  Absorbed from the former
    gen_2fsk.py.  Returns (n_samples, bits)."""
...
def find_packets_multiphase(ch, spb, sync_tol=3, payload_expect=PAYLOAD_EXPECT,
    ...
    0xBEEF (the `gen` subcommand's TX word) with a bit-error tolerance.
...
def cmd_gen(args):
    """Generate the 2FSK test signal (.c8) - absorbed from gen_2fsk.py."""
    if args.random is not None:
        ...
        payload = PAYLOAD_EXPECT
    if not payload:
        sys.exit("payload must not be empty")
    print(f"samples/symbol={int(FS / args.baud)}, "
          f"bits/packet={len(packet_bits(args.preamble, payload))}",
          file=sys.stderr)
    gen_2fsk_signal(args.out, args.repeat, args.gap_ms,
                    preamble_bytes=args.preamble, payload=payload,
                    baud=args.baud, dev=args.dev)
...
def main():
    ...
    g = sub.add_parser("gen", help="generate 2FSK test signal (.c8) for the "
                                   "RFuzz testcase (absorbed from gen_2fsk.py)")
    g.add_argument("out", help="output .c8 file (int8 IQ interleaved)")
    g.add_argument("--repeat", "-n", type=int, default=100,
                   help="number of packets (default 100)")
    g.add_argument("--gap-ms", type=float, default=0.0,
                   help="idle silence between packets in ms (default 0)")
    g.add_argument("--preamble", "-p", type=int, default=DEFAULT_PREAMBLE_BYTES,
                   help="preamble bytes (default 4; >=8 for single-shot bursts)")
    g.add_argument("--baud", type=float, default=BPS,
                   help="symbol rate in bps (default 2400)")
    g.add_argument("--dev", type=float, default=DEFAULT_DEV,
                   help="FSK deviation in Hz (default 50000)")
    gx = g.add_mutually_exclusive_group()
    gx.add_argument("--payload", type=str, default=None,
                    help="payload bytes as hex (default 01020304)")
    gx.add_argument("--random", type=int, nargs=2, default=None,
                    metavar=("LEN", "SEED"),
